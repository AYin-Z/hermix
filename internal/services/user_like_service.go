...
func (s *userLikeService) TopicLike(userId int64, topicId int64) error {
	...
	if err := sqls.WithTransaction(func(ctx *sqls.TxContext) error {
		...
		// 更新点赞数
		return repositories.TopicRepository.UpdateColumn(ctx.Tx, topicId, "like_count", gorm.Expr("like_count + 1"))
	}); err != nil {
		return err
	}
...
func (s *userLikeService) TopicUnLike(userId int64, topicId int64) error {
	...
	if err := sqls.WithTransaction(func(ctx *sqls.TxContext) error {
		if err := s.unlike(ctx.Tx, userId, constants.EntityTopic, topicId); err != nil {
			return err
		}
		// 更新点赞数
		return repositories.TopicRepository.UpdateColumn(ctx.Tx, topicId, "like_count", gorm.Expr("like_count - 1"))
...
func (s *userLikeService) ArticleLike(userId int64, articleId int64) error {
	...
	if err := sqls.WithTransaction(func(ctx *sqls.TxContext) error {
		...
		// 更新点赞数
		return repositories.ArticleRepository.UpdateColumn(ctx.Tx, articleId, "like_count", gorm.Expr("like_count + 1"))
	}); err != nil {
		return err
	}
...
func (s *userLikeService) ArticleUnLike(userId int64, articleId int64) error {
	...
	if err := sqls.WithTransaction(func(ctx *sqls.TxContext) error {
		if err := s.unlike(ctx.Tx, userId, constants.EntityArticle, articleId); err != nil {
			return err
		}
		// 更新点赞数
		return repositories.ArticleRepository.UpdateColumn(ctx.Tx, articleId, "like_count", gorm.Expr("like_count - 1"))
...
func (s *userLikeService) CommentUnLike(userId int64, commentId int64) error {
	...
	if err := sqls.WithTransaction(func(ctx *sqls.TxContext) error {
		if err := s.unlike(ctx.Tx, userId, constants.EntityComment, commentId); err != nil {
			return err
		}
		// 更新点赞数
		return repositories.CommentRepository.UpdateColumn(ctx.Tx, commentId, "like_count", gorm.Expr("like_count - 1"))
...
func (s *userLikeService) like(ctx *sqls.TxContext, userId int64, entityType string, entityId int64) error {
	// 判断是否已经点赞了
	if repositories.UserLikeRepository.Exists(ctx.Tx, userId, entityType, entityId) {
		return errors.New("already liked")
	}
	// 点赞
	return repositories.UserLikeRepository.Create(ctx.Tx, &models.UserLike{
		UserId:     userId,
...
func (s userLikeService) unlike(tx *gorm.DB, userId int64, entityType string, entityId int64) error {
	...
	// 未点赞过则不允许取消：避免 like_count 与作者信誉被刷成负数
