...
(function() {
  ...
  fetch('/api/v3/plugins/hermix/skills')
    .then(function(r) { return r.json(); })
    .then(function(data) {
      var skills = data.response.skills || [];
      var container = document.getElementById('hermix-skills-container');
      if (!skills.length) {
      ...
      container.innerHTML = skills.map(function(s) {
        return '<div class="card p-3">' +
          ...
          '<div class="d-flex gap-2 mt-2">' + (s.tags || []).map(function(t) { return '<span class="badge text-xs">' + esc(t) + '</span>'; }).join('') + '</div>' +
          '<div class="text-xs text-muted mt-2">' +
            '作者：' + esc(s.author_name) + ' · ' + s.rating.toFixed(1) + ' (' + s.rating_count + '评) · ' + s.installs + ' 安装</div>' +
          '</div></div>' +
          (s.install_command ? '<div class="mt-2"><code class="text-xs">' + esc(s.install_command) + '</code></div>' : '') +
        '</div>';
      }).join('');
    })
    .catch(function() { document.getElementById('hermix-skills-container').innerHTML = '<div class="alert p-3">加载失败</div>'; });
})();
</script>
